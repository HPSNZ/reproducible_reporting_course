...
This week's materials are in two parts:

1. In this slidedeck, we'll focus on **how to tidy data**.
2. In the companion slidedeck, we'll work on **creating plots that are commonly used in our NZAJ reports**.
...
We will be using functions from these packages: `{dplyr}`, `{tidyr}`, `{janitor}`, `{stringr}`.

--

If you want to code along as you work through the slides, you'll need to install these packages first.
...
We can build on the code we've already started. After the `clean_names()` function, we add another `%>%` pipe operator. This allows us to "add" the `rename()` function, from the `{dplyr}` package, into our data tidying pipeline:
...
> and assign the data set to an object called `audience_data_tidy`.
...
> apply a filter that keeps those observations where `response_text` does not equal `"N/A"`.  
...
The data class is still showing `character`. That's because, while we've excluded *"N/A"* values, we haven't explicitly re-defined the data class for `response_text`.
