...
.pull-left[
Here's a scrollable table you can use to inspect the `ToothGrowth` data set.
...
.pull-left[
In the code below, we've added the argument `fill = supp` inside the `aes()` function:
...
This means: fill geometric shapes based on their values within the `supp` variable (either `"OJ"` or `"VC"`).
]
...
<small>Note that, by default, `{ggplot2}` displays a legend to define how colours are mapped to data values.</small>
...
.pull-left[
To remove the legend, we add a layer to modify the plot's theme elements.
...
.pull-left[
Use the `labs()` function, specifying the `x` and `y` arguments to assign labels for those axes.
...
<small>Note that I have added the `labs()` layer before the `theme()` layer. A common practice is to specify plot theme elements as the last step in the "background-to-foreground" approach of building `{ggplot2}` plots.</small>
...
I encourage you to have a go at building your plot first, before peeking at the worked examples!
