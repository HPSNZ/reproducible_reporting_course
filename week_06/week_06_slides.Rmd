...
We can use a range of base R functions to retrieve information about the data we've imported into SQL and is now stored in the object called `source_data`.
...
# Print metadata to the console about the structure
# of the data frame object, such as the number of variables,
# the variable names, observation counts, and the
# type of data within each variable.
str(source_data)
...
When we run `str(source_data)`, we see that this data frame includes 695 observations and 6 variables:

- `question_text`: The text for questions that asked respondents to report their date-of-birth in an NZAJ survey.
...
- `age`: Respondent age at the time of survey completion.
